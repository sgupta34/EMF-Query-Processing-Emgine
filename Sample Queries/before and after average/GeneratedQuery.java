...
public class GeneratedQuery 
{
	...
	public static void main(String[] args) 
	{
		...
		mfStruct.put("sum_0_quant", new ArrayList<Integer>());
		mfStruct.put("count_0_quant", new ArrayList<Integer>());
		...
		try 
		{
			...
			while (rs.next()) 
			{
				...
				{
					if(cust.equals((String)mfStruct.get("cust").get(i)+"")&&month.equals((int)mfStruct.get("month").get(i)+"")) 
					{
						flag = false;
						break;
					}
				...
				if(flag) 
				{
					...
					mfStruct.get("sum_0_quant").add(null);
					mfStruct.get("count_0_quant").add(null);
			...
			while (rs.next()) 
			{
				...
				{
					// Sigma(0)
					if((!(mfStruct.get("cust").get(i)+"").equals("null"))&&(!(mfStruct.get("month").get(i)+"").equals("null"))&&(cust.equals((String)mfStruct.get("cust").get(i)+""))&&(month.equals((int)mfStruct.get("month").get(i)+""))) 
					{
						// sum_0_quant
						String _sum_0_quant = mfStruct.get("sum_0_quant").get(i)+"";
						if(_sum_0_quant.equals("null")) 
						{
							_sum_0_quant = "0";
						}
						mfStruct.get("sum_0_quant").set(i, (int)(Double.parseDouble(_sum_0_quant)+Double.parseDouble(quant)));
						// count_0_quant
						String _count_0_quant = mfStruct.get("count_0_quant").get(i)+"";
						if(_count_0_quant.equals("null")) 
						{
							_count_0_quant = "0";
						}
						mfStruct.get("count_0_quant").set(i, (int)(Double.parseDouble(_count_0_quant)+1d));
					}
				}
			}
			...
			while (rs.next()) 
			{
				...
				{
					// Sigma(1)
					if((!(mfStruct.get("cust").get(i)+"").equals("null"))&&(!(mfStruct.get("month").get(i)+"").equals("null"))&&(cust.equals((String)mfStruct.get("cust").get(i)+""))&&(Double.parseDouble(month)<Double.parseDouble((int)mfStruct.get("month").get(i)+""))) 
					{
						// sum_1_quant
			...
			for(int i = 0; i < mfStruct.get("cust").size(); i++) 
			{
				...
				if(true) 
				{
					...
					// avg_0_quant
					if(((mfStruct.get("sum_0_quant").get(i)+"").equals("null"))||((mfStruct.get("count_0_quant").get(i)+"").equals("null"))) 
					{
						System.out.print(String.format("%16s", "null"));
					}
					else 
					{
						System.out.print(String.format("%16.3f", Double.parseDouble((int)mfStruct.get("sum_0_quant").get(i)+"")/Double.parseDouble((int)mfStruct.get("count_0_quant").get(i)+"")));
					}
					// avg_2_quant
